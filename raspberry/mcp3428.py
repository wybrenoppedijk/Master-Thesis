...
class MCP3428():
    ...
    def take_single_reading(self, channel):
        ...
        while float(time.time() < start + 1):
            if ((
                self.smbus.read_i2c_block_data(self.address, self.mode | self.sample_rate | self.gain | (32 * channel))[
                    2]) & 128) == 0:
                reading = self.smbus.read_i2c_block_data(self.address,
                                                         self.mode | self.sample_rate | self.gain | (32 * channel), 2)
